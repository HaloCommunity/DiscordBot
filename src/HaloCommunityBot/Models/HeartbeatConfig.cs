    ...
    public void Validate()
    {
        if (Enabled)
        {
            if (string.IsNullOrWhiteSpace(PushUrl))
                throw new InvalidOperationException("HeartbeatConfig: When enabled, PushUrl is required and cannot be empty. Check HALOCOMMUNITYBOT_Bot__Heartbeat__PushUrl environment variable.");
